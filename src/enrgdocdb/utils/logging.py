...
def setup_logging(app) -> log_module.Logger:
    ...
    def after_request(response):
        level = log_module.INFO if response.status_code < 400 else log_module.WARNING
        logger.log(
            level,
            f"{request.method} {request.path} - {response.status_code} - {getattr(g, 'request_id', None)}",
            extra={
    ...
    def handle_exception(e):
        ...
        # Pass HTTP exceptions through with their proper status codes
        # (abort(404) should return 404, abort(403) should return 403, etc.)
...
class AuditLogger:
    ...
    def permission_denied(
        self, user_email: str, resource_type: str, resource_id: int, action: str
    ):
        self.logger.warning(
